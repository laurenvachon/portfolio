 ...
 * Lauren Vachon
 */
$(document).ready(function(){var d;var c=0;var f=5;var b=$("header").outerHeight();$(window).scroll(function(g){d=true});setInterval(function(){if(d){a();d=false}},50);function a(){var g=$(this).scrollTop();if(Math.abs(c-g)<=f){return}if(g>c&&g>b){$("header").removeClass("nav-down").addClass("nav-up")}else{if(g+$(window).height()<$(document).height()){$("header").removeClass("nav-up").addClass("nav-down")}}c=g}AOS.init({duration:1200,});if($("body").hasClass("homepage")){var e=$("#section1").height();$(window).scroll(function(){if($(this).scrollTop()>e-400){$(".navbar").css("right","0");$(".navbar").addClass("moveOut")}else{$(".navbar").css("right","-80px");$(".navbar").removeClass("moveOut")}});$(".soSmooth").on("click",function(g){if(this.hash!==""){g.preventDefault();var h=this.hash;$("html, body").animate({scrollTop:$(h).offset().top},800,function(){window.location.hash=h})}})}if($("body").hasClass("journal")){$(function(){var g=$("#jsonContent");var h={rss_url:"https://medium.com/feed/@laurenvachon"};$.get("https://api.rss2json.com/v1/api.json",h,function(j){if(j.status=="ok"){var i="";var k=$.map(j.items,function(n,m){var l=j.items[m].categories;if(l.length!==0){return n}});$.each(j.items,function(p,v){var o;if(p<12){o=""}else{o=" visible-sm"}i+='<div class="col-sm-6 col-md-4'+o+'">';i+='<div class="blog-post"><div>';var s=v.description.indexOf("<img");var t=v.description.substring(s).indexOf("src=")+s;var q=t+5;var u=v.description.substring(q).indexOf('"')+q;var l=v.description.substring(q,u);i+='<div class="blog-element"><img class="img-responsive" src="'+l+'" width="360px" height="240px"></div></div>';i+='<div class="blog-content"><h3>'+v.title+"</h3>";i+='<div class="post-meta"><span>By '+v.author+"</span></div>";var n=v.description.replace(/<img[^>]*>/g,"");var m=120;var r=n.substr(0,m);r=r.substr(0,Math.min(r.length,r.lastIndexOf(" ")));i+="<p>"+r+"...</p>";i+='<p><a href="'+v.link+'" target="_blank"> Continue Reading </a></p>';i+="</div></div></div>";return p<12});g.html(i)}})})}if($("body").hasClass("photography")){$("img").on("contextmenu",function(g){return false});$(window).load(function(){$(".grid").fadeIn(3000).masonry({itemSelector:".grid-item",columnWidth:".grid-sizer",percentPosition:true})});$("[data-fancybox]").fancybox({speed:630,image:{protect:true}})}});